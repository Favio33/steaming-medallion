...
from pyspark.sql.functions import current_date, months_between, floor, when, from_json, expr, col, to_date, rank
...
class CDCUpserter:
    ...
    def upsert(self, df_micro_batch, batch_id):
        ...
        (df_micro_batch.filter(col("update_type").isin(["new", "update"]))
...
class Silver():
    ...
    def upsert_users(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        #      - ignoreDeletes allows to delete records on partition column in the bronze layer
        #           without exception on silver layer streams
        ...
        # However, we want to ignoreDeletes, remove duplicates and also merge with an update statement
    ...
    def upsert_user_profile(self, once=False, processing_time="15 seconds", startingVersion=0):
        ...
        df_cdc = (spark.readStream
                  ...
                  .select("v.*")
                  .select("user_id", to_date('dob', 'MM/dd/yyyy').alias('dob'),
    ...
    def upsert_user_bins(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        # Running stream on silver table requires ignoreChange
    ...
    def assert_count(self, table_name, expected_count, filter="true"):
        ...
        assert actual_count == expected_count, f"""Expected {expected_count:,} records,
            found {actual_count:,} in {table_name} where {filter}"""
