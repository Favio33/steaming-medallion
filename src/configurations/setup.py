...
class SetupHelper():   
    ...
    def cleanup(self):
        dfTables = spark.sql(f"SHOW TABLES IN develop.dummy").where(~col("tableName").isin("h_payment_demo", "promotions", "promotions_v2", "user_item_data"))
        if not dfTables.isEmpty():
            print(f"Dropping tables from {self.catalog}.{self.db_name}...", end='')
            for row in dfTables.collect():
                table_name = row["tableName"]
                if table_name != "gym_summary":
                    spark.sql(f"DROP TABLE IF EXISTS {self.catalog}.{self.db_name}.{table_name}")
                    print(f"Tabla {table_name} eliminada.")
                else:
                    spark.sql(f"DROP VIEW IF EXISTS {self.catalog}.{self.db_name}.{table_name}")
                    print(f"Tabla {table_name} eliminada.")
            print("Done")
